...
type proofRsp struct {
	JsonRPC string          `json:"jsonrpc"`
	Result  PixiechainProof `json:"result,omitempty"`
	Error   *jsonError      `json:"error,omitempty"`
	Id      uint            `json:"id"`
}

type PixiechainProof struct {
...
func GetExplorerUrl(chainId uint64) string {
	...
	case uint64(6626): // Pixie Chain Mainnet ChainId
		return "https://scan.chain.pixie.xyz/tx/"
	case uint64(666): // Pixie Chain Testnet ChainId
		return "https://scan-testnet.chain.pixie.xyz/tx/"
	default:
		return "no url"
	}
}
