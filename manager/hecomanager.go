...
func NewPixiechainManager(servconfig *config.ServiceConfig, startheight uint64, startforceheight uint64, ontsdk *sdk.PolySdk, client *ethclient.Client, boltDB *db.BoltDB) (*PixiechainManager, error) {
	...
	if !common.FileExisted(servconfig.PolyConfig.WalletFile) {
		...
		if err != nil {
			log.Errorf("NewPixiechainManager - wallet open error: %s", err.Error())
			return nil, err
		}
	}
	...
	if err != nil || signer == nil {
		...
		if err != nil {
			log.Errorf("NewPixiechainManager - wallet password error")
			return nil, err
		}
	...
	log.Infof("NewPixiechainManager - poly address: %s", signer.Address.ToBase58())

	mgr := &PixiechainManager{
		config:        servconfig,
...
func (this *PixiechainManager) MonitorPixiechainChain() {
	fetchBlockTicker := time.NewTicker(time.Duration(this.config.PixiechainConfig.MonitorInterval) * time.Second)
	var blockHandleResult bool
	for {
		select {
		case <-fetchBlockTicker.C:
			height, err := tools.GetNodeHeight(this.config.PixiechainConfig.RestURL, this.restClient)
			if err != nil {
				log.Infof("MonitorChain - cannot get node height, err: %s", err)
				continue
			}
			if height-this.currentHeight <= config.PIXIECHAIN_USEFUL_BLOCK_NUM {
				continue
			}
			log.Infof("MonitorChain - pixiechain height is %d", height)
			blockHandleResult = true
			for this.currentHeight < height-config.PIXIECHAIN_USEFUL_BLOCK_NUM {
				if this.currentHeight%10 == 0 {
					log.Infof("handle new pixiechain Block height: %d", this.currentHeight)
				}
				...
				if len(this.header4sync) >= this.config.PixiechainConfig.HeadersPerBatch {
					if res := this.commitPixiechainHeaderToPoly(); res != 0 {
			...
			if blockHandleResult && len(this.header4sync) > 0 {
				this.commitPixiechainHeaderToPoly()
			}
		case <-this.exitChan:
			return
		}
...
func (this *PixiechainManager) init() error {
	// get latest height
	...
	log.Infof("PixiechainManager init - start height: %d", this.currentHeight)
	return nil
}

func (this *PixiechainManager) findLastestHeight() uint64 {
	// try to get key
	var sideChainIdBytes [8]byte
	binary.LittleEndian.PutUint64(sideChainIdBytes[:], this.config.PixiechainConfig.SideChainId)
...
func (this *PixiechainManager) CheckIfCommitedToPolyAndParseLockDepositEvent(height uint64) bool {
...
func (this *PixiechainManager) handleBlockHeader(height uint64) bool {
	...
	raw, _ := this.polySdk.GetStorage(autils.HeaderSyncContractAddress.ToHexString(),
		append(append([]byte(scom.MAIN_CHAIN), autils.GetUint64Bytes(this.config.PixiechainConfig.SideChainId)...), autils.GetUint64Bytes(height)...))
...
func (this *PixiechainManager) fetchLockDepositEvents(height uint64, client *ethclient.Client) bool {
	lockAddress := ethcommon.HexToAddress(this.config.PixiechainConfig.ECCMContractAddress)
	...
	for events.Next() {
		...
		raw, _ := this.polySdk.GetStorage(autils.CrossChainManagerContractAddress.ToHexString(),
			append(append([]byte(cross_chain_manager.DONE_TX), autils.GetUint64Bytes(this.config.PixiechainConfig.SideChainId)...), param.CrossChainID...))
		if len(raw) != 0 {
...
func (this *PixiechainManager) commitPixiechainHeaderToPoly() int {
	tx, err := this.polySdk.Native.Hs.SyncBlockHeader(
		this.config.PixiechainConfig.SideChainId,
...
func (this *PixiechainManager) rollBackToCommAncestor() {
	for ; ; this.currentHeight-- {
		raw, err := this.polySdk.GetStorage(autils.HeaderSyncContractAddress.ToHexString(),
			append(append([]byte(scom.MAIN_CHAIN), autils.GetUint64Bytes(this.config.PixiechainConfig.SideChainId)...), autils.GetUint64Bytes(this.currentHeight)...))
...
func (this *PixiechainManager) RegularlyTryCommitPixiechainLockProofToPoly() {
	monitorTicker := time.NewTicker(time.Duration(this.config.PixiechainConfig.MonitorInterval) * time.Second)
	for {
		select {
		case <-monitorTicker.C:
			height, err := tools.GetNodeHeight(this.config.PixiechainConfig.RestURL, this.restClient)
			if err != nil {
				log.Infof("MonitorDeposit - cannot get pixiechain node height, err: %s", err)
				continue
			}
			snycheight := this.findLastestHeight()
			log.Log.Info("MonitorDeposit from pixiechain - snyced pixiechain height", snycheight, "pixiechain height", height, "diff", height-snycheight)
...
func (this *PixiechainManager) handleCachedLockDepositEvents(refHeight uint64) error {
	...
	for _, v := range retryList {
		...
		if refHeight <= crosstx.height+this.config.PixiechainConfig.BlockConfig {
			continue
		}
		height := int64(refHeight - this.config.PixiechainConfig.BlockConfig)
		heightHex := hexutil.EncodeBig(big.NewInt(height))
		proofKey := hexutil.Encode(keyBytes)
		// 2. get proof
		proof, err := tools.GetProof(this.config.PixiechainConfig.RestURL, this.config.PixiechainConfig.ECCDContractAddress, proofKey, heightHex, this.restClient)
		if err != nil {
		...
		if err != nil {
			if strings.Contains(err.Error(), "chooseUtxos, current utxo is not enough") {
				...
				log.Debugf("handleLockDepositEvents - pixiechain_tx %s already on poly", ethcommon.BytesToHash(crosstx.txId).String())
				if err := this.db.DeleteRetry(v); err != nil {
					log.Errorf("handleLockDepositEvents - this.db.DeleteRetry error: %s", err)
				}
				continue
			} else {
				log.Errorf("handleCachedLockDepositEvents - commitProof to poly error for pixiechain_tx %s: %s", ethcommon.BytesToHash(crosstx.txId).String(), err)
				continue
			}
		}
...
func (this *PixiechainManager) commitProof(height uint32, proof []byte, value []byte, txhash []byte) (string, error) {
	...
	if err != nil {
		...
		log.Infof("commitProof - send transaction to poly chain: ( poly_txhash: %s, pixiechain_txhash: %s, height: %d )",
			tx.ToHexString(), ethcommon.BytesToHash(txhash).String(), height)
		return tx.ToHexString(), nil
	}
}
func (this *PixiechainManager) parserValue(value []byte) []byte {
...
func (this *PixiechainManager) CheckDeposit() {
	checkTicker := time.NewTicker(time.Duration(this.config.PixiechainConfig.MonitorInterval) * time.Second)
	for {
		select {
		case <-checkTicker.C:
...
func (this *PixiechainManager) checkLockDepositEvents() error {
