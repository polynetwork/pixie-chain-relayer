...
func NewPolyManager(servCfg *config.ServiceConfig, startblockHeight uint32, polySdk *sdk.PolySdk, ethereumsdk *ethclient.Client, boltDB *db.BoltDB) (*PolyManager, error) {
	...
	ks := tools.NewPixiechainKeyStore(servCfg.PixiechainConfig, chainId)
	accArr := ks.GetAccounts()
	if len(servCfg.PixiechainConfig.KeyStorePwdSet) == 0 {
		fmt.Println("please input the passwords for pixiechain(or ethereum) keystore: ")
		for _, v := range accArr {
			...
			servCfg.PixiechainConfig.KeyStorePwdSet[strings.ToLower(v.Address.String())] = string(raw)
		}
	}
	if err = ks.UnlockKeys(servCfg.PixiechainConfig); err != nil {
		return nil, err
	}
...
func (this *PolyManager) findLatestHeight() uint32 {
	address := ethcommon.HexToAddress(this.config.PixiechainConfig.ECCDContractAddress)
...
func (this *PolyManager) IsEpoch(hdr *polytypes.Header) (bool, []byte, error) {
	...
	eccdAddr := ethcommon.HexToAddress(this.config.PixiechainConfig.ECCDContractAddress)
...
func (this *PolyManager) handleDepositEvents(height uint32) bool {
	...
	for _, event := range events {
		for _, notify := range event.Notify {
			if notify.ContractAddress == this.config.PolyConfig.EntranceContractAddress {
				...
				if uint64(states[2].(float64)) != this.config.PixiechainConfig.SideChainId {
					continue
				}
	...
	if cnt == 0 && isEpoch && isCurr && this.config.PixiechainConfig.EnableChangeBookKeeper {
		sender := this.selectSender()
...
type EthSender struct {
	...
	keyStore     *tools.PixiechainKeyStore
...
func (this *EthSender) sendTxToEth(info *EthTxInfo) error {
	...
	if isSuccess {
		log.Infof("successful to relay tx to pixiechain: (pixiechain_hash: %s, nonce: %d, poly_hash: %s, pixiechain_explorer: %s)",
			hash.String(), nonce, info.polyTxHash, tools.GetExplorerUrl(this.keyStore.GetChainId())+hash.String())
	} else {
		log.Errorf("failed to relay tx to pixiechain: (pixiechain_hash: %s, nonce: %d, poly_hash: %s, pixiechain_explorer: %s)",
			hash.String(), nonce, info.polyTxHash, tools.GetExplorerUrl(this.keyStore.GetChainId())+hash.String())
	}
	return nil
}
...
func (this *EthSender) commitDepositEventsWithHeader(header *polytypes.Header, param *common2.ToMerkleValue, headerProof string, anchorHeader *polytypes.Header, polyTxHash string, rawAuditPath []byte) bool {
	...
	if res {
		log.Debugf("already relayed to pixiechain: ( from_chain_id: %d, from_txhash: %x,  param.Txhash: %x)",
	...
	contractaddr := ethcommon.HexToAddress(this.config.PixiechainConfig.ECCMContractAddress)
		...
		go func() {
			for v := range c {
				if err = this.sendTxToEth(v); err != nil {
					log.Errorf("failed to send tx to pixiechain: error: %v, txData: %s", err, hex.EncodeToString(v.txData))
				}
			}
		}()
	}
...
func (this *EthSender) commitHeader(header *polytypes.Header, pubkList []byte) bool {
	...
	if isSuccess {
		log.Infof("successful to relay poly header to pixiechain: (header_hash: %s, height: %d, pixiechain_txhash: %s, nonce: %d, pixiechain_explorer: %s)",
			hash.ToHexString(), header.Height, txhash.String(), nonce, tools.GetExplorerUrl(this.keyStore.GetChainId())+txhash.String())
	} else {
		log.Errorf("failed to relay poly header to pixiechain: (header_hash: %s, height: %d, pixiechain_txhash: %s, nonce: %d, pixiechain_explorer: %s)",
			hash.ToHexString(), header.Height, txhash.String(), nonce, tools.GetExplorerUrl(this.keyStore.GetChainId())+txhash.String())
	}
	return true
}
...
func (this *EthSender) waitTransactionConfirm(polyTxHash string, hash ethcommon.Hash) bool {
	for {
		...
		log.Debugf("( pixiechain_transaction %s, poly_tx %s ) is pending: %v", hash.String(), polyTxHash, ispending)
		if ispending == true {
			continue
		} else {
